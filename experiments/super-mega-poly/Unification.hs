...
invert gamma sp = do
  ...
  let go :: Spine -> IO (Lvl, IM.IntMap Lvl, Pruning, Bool)
      ...
      go SProj1{} = throwIO UnifyError
      go SProj2{} = throwIO UnifyError
...
rename pren t = case force t of
  ...
  VEx x a b   -> Ex x <$> rename pren a <*> rename (lift pren) (b $$ VVar (cod pren))
...
flexFlex gamma m sp m' sp'
  ...
  | spLen sp < spLen sp' = flexFlex gamma m' sp' m sp
...
unify l t u = case (force t, force u) of
  ...
  (VEx x a b  , VEx x' a' b'   )           -> unify l a a' >> unify (l + 1) (b $$ VVar l) (b' $$ VVar l)
